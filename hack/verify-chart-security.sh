...
# ---------------------------------------------------------------------------
# Leader-election RBAC (setec#217).
#
...
assert_contains "$workdir/le.yaml" "leader election can hold its Lease when enabled" \
	"kind: Role" \
	"coordination.k8s.io" \
	"leases"
assert_contains "$workdir/le.yaml" "the Lease Role is bound to the operator ServiceAccount" \
	"kind: RoleBinding" \
	"kind: ServiceAccount"
